...
损失函数可以深度学习中一个非常重要的概念，用于衡量网络的预测结果与目标值之间的差异，即网络的误差大小。我们一般将预测结果用\(y\)表示，将目标值称为<def>金标准(ground truth,gt)</def>。
...
## MAE
这里我们先简单介绍一个最基本的<def>均值绝对误差</def>, 也就是<def>L1 loss</def>。他用来表示对当前网络的输出y与目标gt之间误差的绝对值进行求和，表示为
...
[//]: # (![]&#40;../img/01/05/MAE.jpg&#41;)
<img alt="MAE" src="../../img/01/05/MAE.jpg" class="img-center" width="50%" />


## MSE
接下来介绍一个和MAE类似的损失函数，<def>均方误差（Mean Square Error, MSE）</def>，也就是<def>L2 loss</def>，他用来表示对当前网络的输出y与目标gt之间误差的绝对值进行求和，表示为
...
[//]: # (![]&#40;../img/01/05/MSE.jpg&#41;)
<img alt="MSE" src="../../img/01/05/MSE.jpg" class="img-center" width="50%" />
...
额是不是要先解释一下什么是<def>离群值（Outlier）</def>，是指在数据集中与其他数据点显著不同或明显偏离的数据点。离群值可能是由于测量误差、数据录入错误、异常事件等原因引起的，它们通常与其他数据点存在较大的差异，会对数据集的统计分析和建模产生影响。

至于为什么MAE鲁棒，比如预测个人资产的数据集，一般人都是十万左右，突然有马云的一百亿，那数据就这样：
...
交叉熵通常用于<def>分类问题</def>中,这里分类任务可以简单理解为目标值具有有限个类别，这里的有限个假如是2，就是二分类，比如给定一个人，分类男或者女。假如大于2，就是<def>多分类</def>，比如给定一个水果的图片，分类是苹果，梨还是橘子。
交叉熵损失函数定义为：
...
!!! Important
	...
	如果图片不属于标签中任何一种类别怎么办？比如给了一个菠萝的图片进行上面的苹果、梨、橘子三分类。这是分类任务固有的弊端，我们暂时不考虑这种情况，


交叉熵的图像如下

[//]: # (![]&#40;../img/01/05/CE.jpg&#41;)
<img alt="CE loss" src="../../img/01/05/CE.jpg" class="img-center" width="50%" />
