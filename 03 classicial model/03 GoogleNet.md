...
GoogLeNet是由Google团队在2014年提出的一个卷积神经网络模型，也被称为Inception V1。GoogLeNet的主要贡献在于提出了一种基于Inception模块的网络结构，可以在保证网络深度和计算效率的同时，提高网络的准确性和泛化能力。
...
为了避免梯度消失，网络额外增加了2个辅助的softmax用于向前传导梯度。这里的辅助分类器只是训练时将三个分类器的损失函数进行加权求和，以缓解梯度消失现象。在正常预测时会被去掉。
