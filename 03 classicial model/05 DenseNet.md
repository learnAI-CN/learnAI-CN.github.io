...
<def>DenseNet（Densely Connected Convolutional Networks）</def>是一种深度卷积神经网络模型，由Zhuang Liu（清华大学）、Gao Huang（康奈尔大学）等人于2017年提出。
DenseNet的主要贡献在于提出了<def>密集连接（dense connection）</def>的思想，可以在保持网络深度和参数数量的同时，提高网络的准确性和泛化能力。
...
## 模型介绍
DenseNet的设计和ResNet类似，也是模块化复用。它包含三个<def>密集块（ Dense Block）</def>,两个相邻块之间的层称为过渡层，由1×1 Conv 和 2×2 平均池化构成。
...
<img alt="Denseblock" src="../../img/03/05/Denseblock.jpg" class="img-center" width="50%" />


因此，第l层有l个输入，这些输入是该层之前的所有<def>卷积块（block）</def>的特征图。如果每个函数 /( H_l /) 都产生 k 个特征图，之后的第层就有  /( k_0 + k*(l-1) /) 
...
之前需要保留的信息会通过短连接保留，两者共同组成<def>"集体知识（collective knowledge）"</def>，即concat后的输入。

!!! note "讨论"

    后面很多工作在讨论**特征融合**时会选择使用Add还是concat。对应的思路背后分别是ResNet和DenseNet。
    
    Add 操作主要用于残差网络中，可以提高模型的收敛速度和泛化能力；Concat 操作主要用于多尺度特征融合中，可以提高模型的感知能力和表达能力。

## 从泰勒展开的角度
这个观点主要来自于李沐大佬。对于任意函数的<def>泰勒展开式（Taylor expansion）</def  >，它把这个函数分解成越来越高阶的项。在 x 接近 0 时，
...
<img alt="compare" src="../../img/03/05/compare.jpg" class="img-center" width="50%" />
...
<img alt="DenseNet" src="../../img/03/05/DenseNet.jpg" class="img-center" width="50%" />


## 从HighWay Network 的角度

HighWay Network是15年一篇通过引入<def>Highway</def>机制，进而有效地训练非常深的网络的论文。
简单来说，论文中基于门机制引入了<def>transform gate T </def>和<def>carry gate C</def>，输出output是由tranform input和carry input组成。
...
相当于是一种<def>隐形的深度监督（implicit deep supervision）</def>。这有助于训练更深的网络

回想一下GoogleNet使用的分类辅助器分枝，迫使中间层也学习判断特征。而之前的<def>深度监督网络（DSN）</def>中在每一隐含层都加了分类器。
