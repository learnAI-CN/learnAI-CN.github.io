...
CLIP（Contrastive Language-Image Pre-Training）是一篇transformer用于<def>多模态</def>的经典论文，当然也可以归于[zero-shot](../08%20Subdivision%20direction/zero%20shot/01%20Paper%20list.md)领域。

CLIP是由OpenAI提出的一种图像和文本联合训练方法，从标题可以看出，他是使用自然语言处理得到的监督信息，训练一个迁移性很好的视觉模型。
...
<img alt="clip train Net" src="../../img/06/06/clip-train.jpg" class="img-center" width="70%" />
...
<img alt="clip train Pseudocode" src="../../img/06/06/clip-train2.jpg" class="img-center" width="50%" />
...
<img alt="clip net infer" src="../../img/06/06/clip-infer.jpg" class="img-center" width="70%" />


网络在预训练好之后，就可以直接在标准数据集上进行测试了。不需要finetune，所以也可以说是zero-shot。测试流程如下

1、对标签进行预处理。因为原始标签可能是单词，比如<var>"cat"</var>，需要对其转化为文本，如<var>"a photo of a cat"</var>。
